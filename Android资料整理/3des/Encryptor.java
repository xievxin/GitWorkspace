public class Encryptor {
  ...
  public byte[] encrypt(byte[] idata) {
    ...
    for (int i = 0; i < processTimes; i++) {
    	 olen = (i+1) * BLOCK_SIZE;
         processBlock(idata, olen-BLOCK_SIZE, buf, olen-BLOCK_SIZE);
...
//  				hs = hs + ":";
  ...
  public static void main(String[] args) {
		try {
			Encryptor en = new Encryptor("11111111".getBytes());
			String str = "1";
			byte[] temp = en.encrypt(str.getBytes());
//			temp = en.encrypt(temp);
//			temp = en.encrypt(temp);
//			byte[] base64 = Base64.encode(temp);
//			byte[] encr = Base64.decode(new String(base64));
			byte[] temp2 = en.decrypt(temp);
			
//			temp2 = en.decrypt(temp2);
//			temp2 = en.decrypt(temp2);
			
			
			String key1 = "11111111";
			String key2 = "44444444";
			String key3 = "33333333";
			
			Encryptor en1 = new Encryptor(key1.getBytes());
			Encryptor en2 = new Encryptor(key2.getBytes());
			Encryptor en3 = new Encryptor(key3.getBytes());
			
			
		  byte[] crypt =en3.encrypt(en2.decrypt(en1.encrypt(str.getBytes())));
		  byte[] plain = en1.decrypt(en2.encrypt( en3.decrypt(crypt)));

		  
		 /* 
			byte[] temp1 = en1.encrypt(str.getBytes());
			byte[] temp_2 = en2.decrypt(temp1);
			byte[] temp_3 = en3.encrypt(temp_2);
			
			
			
		
			byte[] temp11 = en3.decrypt(temp_3);
			byte[] temp_21 = en2.encrypt(temp11);
			byte[] temp_31 = en1.decrypt(temp_21);
